...
export default {
	...
	async getGamesFormatted(serie?: string): Promise<string> {
		...
		const gamesFiltered = games.filter(
			(game) =>
				game.competicao ===
				`Campeonato Brasileiro Série ${serie?.toUpperCase() || 'A'}`
		);
	...
	async updateMemberPosts(cmmId: number, userId: number): Promise<void> {
		...
		const weekNumber = generalFncs.weeksBetween(
			new Date(process.env.INITIAL_DATE),
			new Date()
		);
	...
	getTopicDataFromMessage(
		message: string
	): { cmm: number; tid: number } | null {
	...
	getReminderDate(message: string): Date | false {
		const reminderDateString = message
			...
			.replaceAll(/-[a-z]/gm, '')
			.trim();
		const reminderData = reminderDateString.match(
			/([0-9]*) (minuto|minutos|hora|horas|dia|dias|mês|mes|meses)/m
		);
		...
		const secondsToBeAdded =
			timeInSeconds[reminderData[2]] * parseInt(reminderData[1]);

		const now = new Date();
		const reminderDate = new Date(
			now.setSeconds(now.getSeconds() + secondsToBeAdded)
		);
	...
	async quotePostWithTag(data: ICommandsInput): Promise<void> {
		...
		const quote = !isMessage
			? await this.getQuoteString(postId, userId)
			: '';
		const tag = (message.split('!tag')[1] || message.split('!t')[1])
			?.replace('-m', '')
			.trim();
	...
	async sendGames(data: ICommandsInput): Promise<void> {
		...
		const isMessage = params?.includes('m');
		const serie = ['a', 'b', 'c', 'd'].includes(params?.[0])
			? params[0]
			: undefined;
		...
		const text = games
			? `${quote}\n${games}`
			: `${quote} não encontrei nenhum jogo da série ${serie || 'a'}`;
	...
	async saveToTopic(data: ICommandsInput): Promise<void> {
		...
		const topicTitle = await this.getTopicTitle(cmmId, topicId);
		const { cmm: cmmFromMessage, tid: tidFromMessage } =
			this.getTopicDataFromMessage(message);
