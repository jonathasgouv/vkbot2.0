...
export default {
    async mostPosts (numberOfMembers: number): Promise<void> {
        ...
        cmms.forEach(async cmmId => {
            ...
            const membersSortedByLastWeekPosts = members.sort((a, b) => {
    ...
    async getMostPostsText (members: IMember[]): Promîse<string> {
        ...
        const topMembersResults = members.map((member, index) => `${index + 1}º lugar - ${membersNameResults[index]}, com ${member.posts.slice(-1)[0]} posts.`).join('\n')
