...
const getMostPostsText = async (members: IMember[]): Promise<string> => {
	...
	const topMembersResults = members
		.map(
			(member, index) =>
				`${index + 1}º lugar - ${membersNameResults[index]}, com ${
					member.posts.slice(-1)[0]
				} posts.`
		)
		.join('\n');
	...
	async mostPosts(numberOfMembers: number): Promise<void> {
		...
		cmms.forEach(async (cmmId) => {
			...
			const membersSortedByLastWeekPosts = members
				...
				.filter(
					(member) => member.userId !== parseInt(process.env.BOT_ID)
				);

			const membersWithMostPosts = membersSortedByLastWeekPosts.slice(
				0,
				numberOfMembers
			);
